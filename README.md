...
```
AI-Tool-RAG-Weaviate-Pinecone/
├── gui_app.py                   # GUI приложение (PySide6)
├── run_gui.bat                  # Запуск GUI (единственный способ)
...
**Просто дважды кликните на `run_gui.bat`**
...
### Вкладка "Документы"
1. Введите текст или загрузите файл (TXT, DOCX, PDF)
2. Выберите векторную БД (Pinecone/Weaviate)
3. Нажмите "Добавить документ"
4. Документ автоматически разобьётся на чанки по 200 токенов

### Вкладка "Поиск"
1. Введите ваш вопрос
2. Выберите БД и количество результатов (top_k)
3. Нажмите "Поиск чанков" - увидите релевантные фрагменты
4. Нажмите "Сгенерировать ответ (LLM)" - получите связный ответ от GPT-4o-mini

### Вкладка "Сравнение"
1. Введите запрос
2. Нажмите "Сравнить"
3. Увидите результаты из Pinecone и Weaviate одновременно
...
results = retriever.retrieve("Что такое Python?", "pinecone", top_k=3)
...
### Функциональность
- 🧮 Генерация эмбеддингов через OpenAI (text-embedding-3-large)
- 💾 Поддержка Pinecone и Weaviate
- 🔎 Семантический поиск с настраиваемым top_k
- 🤖 Генерация ответов через GPT-4o-mini
- 📊 Сравнение результатов между БД
- ✂️ Автоматическое разбиение на чанки (200 токенов)
...
- **Эмбеддинги:** OpenAI `text-embedding-3-large` (3072 размерность)
- **Генерация ответов:** OpenAI `gpt-4o-mini` (быстрая и экономичная)
- **Параметры генерации:** temperature=0.3, max_tokens=500
...
```bash
pip install --upgrade openai pinecone-client weaviate-client PySide6 python-docx PyPDF2
```
